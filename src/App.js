...
export function InstancedKeypoints({ geometry, material }) {
  ...
  const [keypoints, setKeypoints] = useSprings(17, (index) => ({
  ...
  const [initialCamDirection, origin] = useMemo(() => {
    return [
      threeData.camera.getWorldDirection().clone(),
    ...
    keypointsRef.current.forEach((keypoint, index) => {
      if (keypoint.score > 0.5) {
        const normalized = new THREE.Vector2(
          ((keypoint.position.x + (window.innerWidth - 950) / 2) /
            window.innerWidth) *
            2 -
            1,
  ...
  const setKeypointsSpring = useCallback((results) => {
    ...
    poseResults.current = results[0].pose.keypoints.map((point, i) =>
      point.score > 0.8
        ? [point.position.x, point.position.y]
        : poseResults.current[i]
    );
    setKeypoints((index) => ({
      point: poseResults.current[index],
      onFrame: ({ point }) => {
  ...
  const onPose = useCallback((results) => {
    if (results.length === 0) return;
    console.log(results[0].pose.keypoints.map((k) => k.part).join(","));
